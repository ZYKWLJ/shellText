...
void search_word(char *str, int library_index, int search_mod, int first_show, int second_show, int third_show)
{
    ...
    {
        // printf("%s%s查询所有词库\n", BLUE, BOLD);
        total = search_in_single_library(library[0] /*单库二维数组*/, str, 0, search_mod, first_show, second_show, third_show) + search_in_single_library(library[1], str, 1, search_mod, first_show, second_show, third_show);
    }
    else
    {
        ...
        // if (single_library == NULL)
        // {
        //     printf("single_library is NULL for lib_index = %d\n", lib_index);
        //     return;
        // }
        total = search_in_single_library(library[library_index], str, library_index, search_mod, first_show, second_show, third_show);
    }
    if (total == 0)
    {
        printf("%s%s没有找到单词:(%s\n", RED, BOLD, RESET);
