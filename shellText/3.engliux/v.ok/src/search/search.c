...
int words_len(word_entry *single_library)
{
    ...
    int len = 0;
    while (single_library[len].index!=NULL)
    {
        len++;
    }
...
void search_in_single_library(word_entry **single_library/*单个库只需二维数组*/, char *str, int lib_index, int search_mod, int first_show, int second_show, int third_show)
{
    printf("查询%d级库\n", lib_index);
...
void search_word(char *str, int library_index, int search_mod, int first_show, int second_show, int third_show)
{
    ...
    {
        printf("%s%s查询所有词库\n", BLUE, BOLD);
        search_in_single_library(library[0]/*单库二维数组*/, str, 0, search_mod, first_show, second_show, third_show);
